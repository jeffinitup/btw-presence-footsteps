	...
	private String resolvePrimitive(int block, int metadata)
	{
		...
		String substrate =
			String.format(
					Locale.ENGLISH,
					"%.2f_%.2f",
					registered.stepSound.stepSoundVolume,
					registered.stepSound.stepSoundPitch
			);
		
		String primitive = this.isolator.getPrimitiveMap().getPrimitiveMapSubstrate(soundName, substrate);
		PFHaddon.debug(soundName + "@" + substrate);
		...
		if (primitive == null)
		{
			...
			PFHaddon.debug(soundName + "break_" + registered.stepSound.getBreakSound());
