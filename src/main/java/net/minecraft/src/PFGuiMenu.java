...
public class PFGuiMenu extends GuiScreen
{
	...
	public void initGui()
	{
		...
		final int _MIX = _GAP + _MARGIN + _UNIT;
		
		final int _LEFT = this.width / 2 - _WIDTH / 2;
		final int _RIGHT = this.width / 2 + _WIDTH / 2;

		final int _TOP = _MIX;
		final int _BOTTOM = this.height - _MIX;
		
		int id = 0;
		
		{
			HGuiSliderControl sliderControl =
				new HGuiSliderControl(id, _LEFT, _TOP, _WIDTH, _UNIT, "", this.mod.getConfig().getInteger(
		...
		this.buttonList.add(new GuiButton(200, _LEFT + _MIX, _BOTTOM - _MARGIN, _WIDTH - _MIX * 2 - _GAP, _UNIT, "Done"));
	...
	public void drawScreen(int par1, int par2, float par3)
	{
		...
		drawCenteredString(this.fontRenderer, this.screenTitle, this.width / 2, 16, 0xffffff);
